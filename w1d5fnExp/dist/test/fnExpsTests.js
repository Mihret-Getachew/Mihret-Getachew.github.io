...
describe("myMap", function () {
    const testArray = [-10, 0, 10, 20];
    it("tests myMap on triples anonyme43ous function", function () {
        assert.deepStrictEqual(myMap(testArray, function (elem) {
            let x = elem * 3;
            return x;
        }), [-30, 0, 30, 60]);
    });
    it("tests myMap on triples arrow function", function () {
        assert.deepStrictEqual(myMap(testArray, (num) => {
            let x = num * 3;
            return x;
        }), [-30, 0, 30, 60]);
    });
