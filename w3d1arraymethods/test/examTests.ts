...
import { firstLast, replaceEnds, square, higho, absVal } from "../src/exam.js";
import {
  calculateDailyTotalDuration,
...
describe("exam2 coding", function () {
  ...
  it("tests absVal", function () {
    ...
    const numArr2: number[] = [-1, 4, -10];

    assert.deepEqual(higho(absVal, numArr1), [1, 2, 3] as number[]);
...
describe("fitness tracking", function () {
  ...
  it("getAllSessionsForUser", function () {
    assert.deepEqual(getAllSessionsForUser(dailyRecord, 2), [
      session2,
      session3,
    ]);
  });
